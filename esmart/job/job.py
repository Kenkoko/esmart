...
class Job:
    ...
    @staticmethod
    def create(
        config: Config, dataset: Optional[Dataset] = None, parent_job=None, builder=None
    ):
        "Create a new job."
        from esmart.job import TrainingJob, SearchJob #, EvaluationJob
        ...
        if job_type == "train":
            return TrainingJob.create(
                config, dataset, parent_job=parent_job, builder=builder
            )
    ...
    def create_from(
        ...
        if "builder" in checkpoint and checkpoint["builder"] is not None:
            builder = BaseBuilder.create_from(
        ...
        job = Job.create(config, dataset, parent_job, builder)
...
class TrainingOrEvaluationJob(Job):
    ...
    def _parse_func(self, filepath, label):
