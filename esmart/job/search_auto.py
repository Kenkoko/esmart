...
from esmart.job import SearchJob, Job
import esmart.job.search
import copy
import gc
...
class AutoSearchJob(SearchJob):
    ...
    def _load(self, checkpoint):
        self.config.log('load checkpoint')
    ...
    def _run(self):
        ...
        while trial_no < self.num_trials:
            ...
            if trial_id is None:
                ...
                config.set_all(copy.deepcopy(parameters))
                config.init_folder()
                ...
                self.submit_task(
                    esmart.job.search._run_train_job,
